...
/// Limits crawl depth via `meta['depth']`.
class DepthMiddleware<T, Req extends Request, Res extends Response> extends ProcessorMiddleware<T, Req, Res> {
  final int maxDepth;
  const DepthMiddleware({this.maxDepth = 5});

  @override
  Stream<Result<T, Req>> onOutput(Res response, Stream<Result<T, Req>> results) async* {
    ...
    await for (final result in results) {
      if (result is FollowResult<T, Req>) {
        ...
        if (nextDepth <= maxDepth) {
          yield Result.follow(result.request.copyWith(meta: {...result.request.meta, 'depth': nextDepth}) as Req);
        }
      } else {
        yield result;
      }
