...
sealed class DMResult<Req extends Request, Res extends Response> {
  ...
  factory DMResult.fail(Object error, [StackTrace? stackTrace]) =
      ReportError<Req, Res>;
  factory DMResult.drop() = DropRequest<Req, Res>;
}

final class ContinueChain<Req extends Request, Res extends Response>
    extends DMResult<Req, Res> {
  final Req request;
  const ContinueChain(this.request);
}

final class ForwardResponse<Req extends Request, Res extends Response>
    extends DMResult<Req, Res> {
  final Res response;
...
final class DropRequest<Req extends Request, Res extends Response>
    extends DMResult<Req, Res> {
  const DropRequest();
}
...
abstract class DownloaderMiddleware<Req extends Request, Res extends Response> {
  ...
  Future<DMResult<Req, Res>> processRequest(Req request) async =>
      DMResult.continueWith(request);

  Future<DMResult<Req, Res>> processResponse(Req request, Res response) async =>
      DMResult.respond(response);

  Future<DMResult<Req, Res>> processException(Req request, Object exception) async =>
      DMResult.fail(exception);
